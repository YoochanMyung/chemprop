...
def run(kwargs):
    ...
    csv_dir = '/clusterdata/uqymyung/uqymyung/projects/deeppk/1_dataset/admetlab2' # WIENER

    if check_categorical(os.path.join(csv_dir,f'{kwargs.endpoint}_test.csv')):
        best_run = fetch_result({'target': kwargs.endpoint, 'r': False, 'run_name': kwargs.run_name, 'prefix': kwargs.prefix})
    else:
        best_run = fetch_result({'target': kwargs.endpoint, 'r': True, 'run_name': kwargs.run_name, 'refix': kwargs.prefix})
    ...
    result_json = json.load(open(os.path.join(f'/clusterdata/uqymyung/uqymyung/projects/deeppk/2_ML/1_Chemprop/after_hyperopt/admetlab2/{kwargs.endpoint}_best.json'), 'r')) # WIENER
